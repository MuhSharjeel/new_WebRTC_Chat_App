...
document.getElementById("join").addEventListener("click", (event) =>{
    ...
    var x = document.getElementById("myText").value;
        ...
        if (x.length == 0){
            alert("You has Joined as an Anoynomus User");
            x="Anoynomus"
        }
        document.getElementById("demo").innerHTML = x + " Welcome to Chat App";
        document.getElementById("show").addEventListener("click", (event) =>{
        socket.emit("active_user",x);})
...
navigator.mediaDevices.getUserMedia({ video: true, audio: true })
    .then(stream => {
        ...
        socket.on("ActiveUser", ShowActiveUsers)
